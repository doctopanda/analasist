...
weeks = pd.to_numeric(ctx.sinave.loc[pd.to_numeric(ctx.sinave["Año"], errors="coerce").eq(year), "SemanaInicio"], errors="coerce")
def_cut = int(weeks[weeks.between(1, 53)].max()) if weeks[weeks.between(1, 53)].notna().any() else 53
cutoff = st.sidebar.slider("Semana epidemiológica de corte", 1, 53, def_cut)
...
fig = px.line(view, x="Semana", y="Casos", color="Año", markers=True,
              color_discrete_sequence=PLOTLY_COLORS, title="Casos por semana epidemiológica")
