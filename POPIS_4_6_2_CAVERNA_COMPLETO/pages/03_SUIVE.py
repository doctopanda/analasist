...
from modulos.suive import cumulative_at_cutoff, matrix
...
hero("SUIVE / SUAVE", subtitle="Vigilancia convencional · magnitud poblacional y tendencias",
     cutoff=ctx.cutoff_label, badges=["Convencional", "Semanal", "No sumar con SINAVE"])
...
latest = years[-1]
def_cut = int(ctx.suive.loc[ctx.suive["Año"].eq(latest), "Semana"].max())
