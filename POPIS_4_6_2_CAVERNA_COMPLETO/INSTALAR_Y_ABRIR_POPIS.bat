...
echo ===============================================================
echo  POPIS 4.6.2 CAVERNA - INSTALACION AUTOMATICA COMPLETA
echo  SUIVE + SINAVE + CANALES + INDICADORES + TERRITORIO
echo ===============================================================
echo.
...
echo [1/6] Python localizado: %PY_CMD%

if not exist "%VENV%\Scripts\python.exe" (
  echo [2/6] Creando entorno virtual aislado en:
  echo       %VENV%
  if not exist "%LOCALAPPDATA%\POPIS462" mkdir "%LOCALAPPDATA%\POPIS462"
  %PY_CMD% -m venv "%VENV%"
  if errorlevel 1 goto :error_venv
) else (
  echo [2/6] Entorno virtual POPIS ya existe.
)

echo [3/6] Instalando/verificando dependencias...
"%VENV%\Scripts\python.exe" -m pip install --disable-pip-version-check --upgrade pip
if errorlevel 1 goto :error_pip
"%VENV%\Scripts\python.exe" -m pip install --disable-pip-version-check -r requirements.txt
if errorlevel 1 goto :error_pip

echo [4/6] Preparando estructura y buscando datos POPIS existentes...
...
echo [5/6] Verificando codigo POPIS...
"%VENV%\Scripts\python.exe" -m py_compile app.py modulos\io_utils.py modulos\sinave.py modulos\suive.py modulos\canal_endemico.py modulos\comparativo.py modulos\indicadores.py modulos\territorio.py modulos\calidad.py modulos\exportacion.py modulos\runtime.py modulos\tema_caverna.py
if errorlevel 1 goto :error_codigo

echo [6/6] Abriendo POPIS 4.6.2...
...
:error_codigo
echo.
echo ERROR: la verificacion del codigo fallo.
