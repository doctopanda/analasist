...
from modulos.io_utils import classify_file, discover_assets, ensure_structure, process_inbox, read_table
...
from modulos.territorio import _read_population_source, build_centroid_map, incidence_table, map_html_bytes, normalize_population
...
def make_rows(year: int, weeks: range, prefix: str) -> list[dict]:
    ...
    for week in weeks:
        ...
        rows.append({
            "Fec_captura": f"15/06/{year}", "Fec_Prim_Contacto": f"14/06/{year}", "Fec_Dx_Final": f"20/06/{year}",
            "Fecha_Inicio": f"12/06/{year}", "SemanaInicio": week, "Sem_Notif": week, "Folio": f"{prefix}-{week}",
            "Mun_Res": municipality, "CVE_EDO_RES": 26, "CVE_MPO_RES": 30 if municipality == "HERMOSILLO" else 18,
            ...
            "Salmonella": "Positivo" if week % 3 == 0 else "", "Shigella": "Positivo" if week % 5 == 0 else "",
            "Rotavirus": "", "VibrioCholerae": "Positivo" if week == first_week else "",
            "SerogrupoVibrioCholerae": "NO O1" if week == first_week else "", "VibrioParahaemolyticus": "",
            "Patotipo_EColi_InDRE": "", "Rotavirus_InDRE": "", "OtroVirus_InDRE": "",
            "MuestraVibrio": "Hisopo rectal" if week == first_week else "", "FecRecepLab": f"15/06/{year}" if week == first_week else "",
...
def write_population_book(path: Path) -> None:
    rows = []
    for clave, mun, total in [("26030", "HERMOSILLO", 1000000), ("26018", "CAJEME", 450000), ("26043", "NOGALES", 280000)]:
...
def main() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write_tsv(root / "data/actual" / "Diarreas_actual.xls", make_rows(2026, range(1, 13), "26") + make_rows(2026, range(53, 54), "26F"))
        ...
        print("DEBUG_SHEETS=", pd.ExcelFile(pop_path, engine="openpyxl").sheet_names)
        direct = pd.read_excel(pop_path, sheet_name="Sonora", engine="openpyxl")
        print("DEBUG_DIRECT_COLS=", list(direct.columns))
        print("DEBUG_DIRECT_ROWS=", direct.head(3).to_dict("records"))
        via_read_table = read_table(pop_path, sheet_name="Sonora")
        print("DEBUG_READ_TABLE_COLS=", list(via_read_table.columns))
        candidate = _read_population_source(pop_path)
        print("DEBUG_CANDIDATE=", None if candidate is None else {"cols": list(candidate.columns), "n": len(candidate), "head": candidate.head(2).to_dict("records")})
        ...
        assert len(map_html_bytes(build_centroid_map(base[(base["Año"].eq(2026)) & pd.to_numeric(base["SemanaInicio"], errors="coerce").le(12)], geo))) > 5000
        ...
        assert len(excel_bytes(tables)) > 3000 and len(word_bytes("POPIS test", tables)) > 10000 and len(html_bytes("POPIS test", tables)) > 1000
        ...
        assert any("Integrado sinave" in m for m in process_inbox(root))
