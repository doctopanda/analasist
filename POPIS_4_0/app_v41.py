...
from popis_data import (
    add_historical, load_preloaded_sources, save_current, save_population,
)
...
st.markdown('<p class="popis-sub">SUIVE + SINAVE + indicadores + territorio + control de calidad · Sonora</p>', unsafe_allow_html=True)
...
# ------------------------- Fuentes locales y actualización --------------------
...
with st.sidebar:
    ...
    st.caption("Los históricos permanecen congelados. Solo reemplaza las bases actuales cada semana.")
    ...
    with st.expander("🗓️ Mantenimiento anual / inicial", expanded=False):
        ...
        new_pop=st.file_uploader("Reemplazar CONAPO",type=["xls","xlsx","csv"],key="pop_add")
        if new_pop and st.button("Guardar CONAPO",use_container_width=True): save_population(new_pop); st.success("CONAPO actualizado"); st.rerun()

# Usar año epidemiológico para análisis semanal SINAVE.
...
available_years=sorted(sin_years|su_years)
...
with c1: year=st.selectbox("Año epidemiológico",available_years or [2026],index=(len(available_years)-1 if available_years else 0))
...
if suive.empty and sinave.empty:
    st.warning("No hay fuentes locales. Usa 'Mantenimiento anual / inicial' para cargarlas una sola vez.")

images={}; export_tables={}
TABS=st.tabs(["🏠 Resumen","📘 SUIVE","🧬 SINAVE","🔄 Comparador","📈 Canal endémico","🗺️ Territorio","☠️ Mortalidad","🧮 Indicadores","🔍 Calidad y auditoría","📦 Exportar"])
...
with TABS[0]:
    ...
    cols=st.columns(6)
    vals=[("SUIVE acumulado",su.get("acumulado")),(f"SUIVE SE{week}",su.get("semana")),("SINAVE acumulado",si.get("acumulado")),(f"SINAVE SE{week}",si.get("semana")),("Patógeno identificado",si.get("positivos_acum")),("Defunciones",si.get("defunciones_acum"))]
    for col,(label,val) in zip(cols,vals): col.metric(label,"—" if pd.isna(val) else f"{int(round(float(val))):,}")
    if not sinave.empty:
        q=quality_issues(sinave,year,week); qs=quality_summary(q); export_tables["Calidad resumen"]=qs
        severe=int(q["Severidad"].isin(["Error","Revisar"]).sum()) if not q.empty else 0
        if severe: st.warning(f"🔎 POPIS detectó {severe:,} observaciones que conviene revisar. Ve a 'Calidad y auditoría'.")
    if not suive.empty:
        cur=suive[(suive["Año"]==year)&(suive["SE"]<=week)]; fig=line_fig(cur,"SE",["Casos"],f"SUIVE · {year}")
        st.pyplot(fig,use_container_width=True); register_fig("⬇️ PNG · SUIVE",f"SUIVE_{year}_SE{week}.png",fig,"r_su",images); plt.close(fig)
    if not sinave.empty:
...
with TABS[2]:
    ...
    if sinave.empty: st.warning("No se detectó SINAVE local.")
        ...
        wk=sinave_weekly(sinave,year); export_tables["SINAVE semanal"]=wk
        fig=line_fig(wk[wk.SE<=week],"SE",["Casos"],f"SINAVE · {year}"); st.pyplot(fig,use_container_width=True); register_fig("⬇️ PNG",f"SINAVE_{year}_SE{week}.png",fig,"si_curve",images); plt.close(fig)

with TABS[3]:
    st.subheader("SUIVE ↔ SINAVE")
    st.caption("La razón entre sistemas es descriptiva; no equivale automáticamente a subregistro.")
...
with TABS[4]:
    ...
    source=st.radio("Fuente",["SUIVE","SINAVE"],horizontal=True,key="ch_src")
    if source=="SUIVE" and not suive.empty:
        hist=[int(y) for y in sorted(suive.Año.unique()) if int(y)<year]; default=[y for y in hist if y not in (2020,2021)] or hist
        selected=st.multiselect("Años históricos",hist,default=default)
        if selected:
            ch=endemic_channel(suive,year,selected,week); export_tables["Canal SUIVE"]=ch; fig=channel_fig(ch,f"Canal endémico SUIVE · {year}"); st.pyplot(fig,use_container_width=True); register_fig("⬇️ PNG",f"Canal_SUIVE_{year}.png",fig,"ch_su",images); plt.close(fig)
            row=ch[ch.SE.eq(week)];
            if not row.empty: st.metric(f"Zona SE{week}",row.iloc[0]["Zona"])
    elif source=="SINAVE" and not sinave.empty:
        paths=["Todos los casos"]+sorted(c.replace("path_","") for c in sinave.columns if c.startswith("path_")); pathogen=st.selectbox("Patógeno",paths)
        hist=[int(y) for y in sorted(pd.to_numeric(sinave.epi_year,errors="coerce").dropna().unique()) if int(y)<year]; selected=st.multiselect("Años históricos SINAVE",hist,default=hist)
        muni=["Todos"]+sorted(sinave.loc[sinave.sonora_resident,"municipality"].dropna().astype(str).unique()); mun=st.selectbox("Municipio",muni)
        if selected:
...
with TABS[5]:
    ...
    if sinave.empty: st.warning("SINAVE no disponible.")
        ...
        terr=territorial_summary(sinave,year,week,population if not population.empty else None); export_tables["Territorio municipal"]=terr
        level=st.radio("Nivel",["Municipio","Distrito","Región"],horizontal=True)
        table=terr if level=="Municipio" else aggregate_territory(terr,level); st.dataframe(table,hide_index=True,use_container_width=True)
        ...
        if metrics:
            ...
            fig=bar_fig(table.head(25),level,metric,f"{level} · {metric}",True); st.pyplot(fig,use_container_width=True); register_fig("⬇️ PNG",f"Territorio_{level}_{metric}_{year}.png",fig,"terr",images); plt.close(fig)
            if level=="Municipio":
                try:
                    geo=fetch_sonora_geojson(); fmap=px.choropleth(terr,geojson=geo,locations="Municipio",color=metric,featureidkey="properties.nomgeo",hover_name="Municipio",projection="mercator",title=f"Sonora · {metric}")
        ...
        if population.empty: st.info("CONAPO no está cargado: se muestran conteos, no tasas.")
...
with TABS[6]:
    ...
    if not sinave.empty:
        ...
        mort=pd.DataFrame(rows); st.dataframe(mort,hide_index=True,use_container_width=True); export_tables["Mortalidad"]=mort
        fig=bar_fig(mort,"Año","Defunciones",f"Defunciones ≤SE{week}"); st.pyplot(fig,use_container_width=True); register_fig("⬇️ PNG",f"Mortalidad_SE{week}.png",fig,"mort",images); plt.close(fig)

with TABS[7]:
    st.subheader("Indicadores normativos EDA")
    st.caption("Los indicadores mensuales utilizan fecha de captura; el análisis semanal utiliza año epidemiológico.")
    if not sinave.empty:
        ind=nutrave_indicators(sinave,year,month); lab=laboratory_indicators(sinave,year,month); export_tables["Indicadores NuTraVE"]=ind; export_tables["Indicadores laboratorio"]=lab
        st.markdown("#### 🏥 NuTraVE"); st.dataframe(ind,hide_index=True,use_container_width=True); st.markdown("#### 🔬 Laboratorio"); st.dataframe(lab,hide_index=True,use_container_width=True)

with TABS[8]:
    st.subheader("Calidad de datos y auditor normativo")
    if not sinave.empty:
        issues=quality_issues(sinave,year,week); qsum=quality_summary(issues); export_tables["Calidad detalle"]=issues; export_tables["Calidad resumen"]=qsum
        st.markdown("#### 🧹 Bitácora de calidad")
        st.dataframe(qsum,hide_index=True,use_container_width=True)
        severities=["Todos"]+sorted(issues.Severidad.unique().tolist()) if not issues.empty else ["Todos"]; sev=st.selectbox("Filtrar severidad",severities)
        shown=issues if sev=="Todos" else issues[issues.Severidad.eq(sev)]; st.dataframe(shown,hide_index=True,use_container_width=True,height=340)
        st.info("Cruce de año epidemiológico es informativo: SE52/53 al inicio de enero no se modifica; POPIS ajusta el año epidemiológico asociado.")
        st.markdown("#### 🔍 Definición operacional EDA")
        audit=audit_operational_definitions(sinave,year,week); export_tables["Auditoría EDA"]=audit; review=audit[audit.Concordante.eq("Revisar")]
        a,b=st.columns(2); a.metric("Evaluados",f"{len(audit):,}"); b.metric("Revisar clasificación",f"{len(review):,}"); st.dataframe(review if len(review) else audit.head(50),hide_index=True,use_container_width=True)
...
st.divider(); st.caption(f"POPIS {BUILD} · datos nominales locales · no publicar bases SINAVE con identificadores personales")
